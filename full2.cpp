...
void searchCSVBySymptoms(const vector<vector<string>>& data, const vector<string>& symptoms) {
    ...
    for (const auto& row : data) {
        ...
        if (allSymptomsFound) {
            found = true;
            cout << "Disease: " << row[0] << endl;
            cout << "Symptoms: ";
            for (int i = 1; i < row.size(); ++i) {
                cout << row[i] << ", ";
            }
            cout << endl;
            break; // Assuming you want to display the first match only
        }
    }
...
string generateRecommendation(const vector<string>& symptoms) {
...
void handleUserInput() {
    ...
    // Convert the symptoms string to a vector of strings
    vector<string> symptomList;
    stringstream ss(symptoms);
    string symptom;
    while (getline(ss, symptom, ',')) {
        symptomList.push_back(toLowerCase(symptom));
    }

    // Process the symptoms and generate a recommendation
    string recommendation = generateRecommendation(symptomList);
